...
@section('facebook_meta')
    <meta property="og:url" content="http://canhgiamgia.com/lam-dep">
    <meta property="og:image" content="{{url('coupon/logo.png')}}">
    <meta property="og:description" content="Canhgiamgia.com | Khuyến mại mỹ phẩm mới nhất từ các shop">
    <meta property="og:title" content="Canhgiamgia.com | Khuyến mại mỹ phẩm mới nhất từ các shop">
@endsection
        ...
        <div class="row">
            <div class="col-md-3">
                <aside class="sidebar-left">
                    ...
                    @endif
                    <ul class="nav nav-tabs nav-stacked nav-coupon-category nav-coupon-category-left">
                        <?php $providers = \App\Deal\Functions::getSources(config('constants.JAMJA_MP'));?>

                        @foreach($providers as $provider)
                            <li><a href="{{url('khuyen-mai/nguon', ['alias'=>str_slug($provider->alias), 'category_id'=>config('constants.JAMJA_MP')]) }}">{{$provider->alias}}<span>{{\App\Deal\Functions::countDealBySource($provider->alias)}}</span></a>
                            </li>
                        @endforeach
